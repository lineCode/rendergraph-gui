...
// CONNECTIONS
int StandardNetworkModel::connectionCount(
    const QModelIndex &connectorIndex) const {
...
void StandardNetworkModel::removeConnectionsOnConnector(
  ...
  auto c = qobject_cast<ConnectorPrivate *>(
...
bool StandardNetworkModel::removeConnection(const QModelIndex &fromConnector,
                                            const QModelIndex &toConnector) {
  // auto srcN = derefNode( static_cast<Node
  // *>(fromConnector.internalPointer()); auto dstN = static_cast<Node
  // *>(toConnector.internalPointer());
  ...
  beginRemoveRows(fromConnector, si, si);
  srcConn->connectedTo.removeAt(si);
  endRemoveRows();

  beginRemoveRows(toConnector, di, di);
